...
function pre([string]$file) {
    ...
    function pre_file([string]$file) {
        ...
        % { 
            if ($mode -ne "normal" ) {
                ...
                if ($_ -match "(.*)`{([a-zA-Z]+):(.*):}(.*)") {
                    $codeblock = $matches[3]
                    $result = pre-eval $matches[2] $codeblock
                    #to ensure the text before and after the code block can
                    #be put on the same line as the result of the code block
                    #we need to do some fancy work here.
                    echo ($matches[1] + $result + $matches[4])
                    $codeblock = ""
                    #pre-eval $matches[1] $matches[2]
                } elseif ($_.Trim().EndsWith("{powershell:")) {
                    #start of a powershell block... (skip this line)
                    $mode = "powershell";
                } elseif ($_.Trim().EndsWith("{sql:")) {
                    #start of a sql block... (skip this line)
                    $mode = "sql";
                } elseif ($_.Trim().EndsWith("{nt:")) {
                    $mode = "nimbleText";
                } else {
    ...
    function pre-eval($mode, $codeblock) {
        if ($codeblock -eq "") {
            return;
        }
        ...
        #if debug write-host $codeblock;
        $x = Invoke-expression $codeblock; 
        return $x
    }
    
    function prepare-Sql($code) {
        # Note: provide connection string via $env:conn.
        # (we parse out the server, db, and either SSPI or username/password... fragile...)
        if ($env:conn -eq $null) {
            write-error "Pre {sql: requires a 'conn' environment variable to specify the connection string"
        }
    ...
    #PRE -- we are back in the scope of the PRE function now.
    
    if ($file -eq "") { 
        #write-error "specify a file to process. for help: pre_help"; return; 
        dir . *.pre -recurse | % { pre $_.FullName } 
    } else {
        
        if (!(Test-Path $file)) {
            write-error ("File not found: '" + $file + "'") -ErrorID 5
            return
        }
        
        $_currentPath = (pwd).path.ToLower()
        
        
        if ($file.ToLower().StartsWith($_currentPath)) {
            write-output (" => " + $_.Name)    
        } else {
            write-output (" => " + $file)
        }
